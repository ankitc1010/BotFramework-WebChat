...
const MessagePaneView = (props: MessagePaneProps) => (
	<div
		className={classList(
			'wc-message-pane',
			props.activityWithSuggestedActions && 'show-actions'
		)}>
		{props.children}
...
export const MessagePane = connect(
	(state: ChatState) => ({
		...
		activityWithSuggestedActions: activityWithSuggestedActions(
	...
	{
		takeSuggestedAction: (message: Message) =>
			({ type: 'Take_SuggestedAction', message } as ChatActions),
		// only used to create helper functions below
		sendMessage
	},
