...
export const history: Reducer<HistoryState> = (
	...
	switch (action.type) {
		...
		case 'Receive_Message':
			if (
				state.activities.find(a => a.id === action.activity.id) ||
			...
			return {
				...
				activities: [
					...
					...state.activities.filter(
						activity =>
							activity.from.id !== action.activity.from.id &&
			...
			return {
				...
				activities: [
					...state.activities.filter(activity => activity.type !== 'typing'),
					{
						...
						channelData: {
							clientActivityId:
								state.clientActivityBase + state.clientActivityCounter
						}
					},
					...state.activities.filter(activity => activity.type === 'typing')
				],
		...
		case 'Send_Message_Retry': {
			...
			const newActivity =
				activity.id === undefined ? activity : { ...activity, id: undefined }
			return {
				...state,
				activities: [
					...state.activities.filter(
						activityT => activityT.type !== 'typing' && activityT !== activity
					),
					newActivity,
					...state.activities.filter(activity => activity.type === 'typing')
				],
		...
		case 'Take_SuggestedAction':
			const i = state.activities.findIndex(
				activity => activity === action.message
			)
			const activity = state.activities[i]
			const newActivity = {
				...activity,
...
const sendMessage: Epic<ChatActions, ChatState> = (action$, store) =>
	action$.ofType('Send_Message').map(action => {
		...
		const clientActivityId =
			state.history.clientActivityBase +
	...
	action$
		...
		.mergeMap(action => {
			...
			if (action.autoListenAfterSpeak) {
				...
				onSpeakingFinished = () =>
					({ type: 'Listening_Starting' } as ShellAction)
			}

			const call$ = speakObservable(
				action.ssml,
				action.locale,
				onSpeakingStarted
			)
		...
		.merge(
			action$
				.ofType('Speak_SSML')
				.map(_ => ({ type: 'Listening_Stop' } as ShellAction))
		)
	...
	action$
		...
		.do((action: ShellAction) => {
			...
			var onFinalResult = (srText: string) => {
				...
				store.dispatch(
					sendChatMessage(srText, store.getState().connection.user, locale)
				)
			}
...
const listeningSilenceTimeout: Epic<ChatActions, ChatState> = (
	action$,
	store
) => {
...
const showTyping: Epic<ChatActions, ChatState> = action$ =>
	action$
		...
		.map(
			action =>
				({ type: 'Clear_Typing', id: action.activity.id } as HistoryAction)
		)
